...
class Board: 
    ...
    #This converts the board into a displayable string. You want it to show the current state of the game. 
    def __str__(self):
        board_str = ""
        for i, row in enumerate(self.board_array):
            dispaly = [x if x != "" else " " for x in row]

            board_str +=  " " + " | ".join(dispaly) + " "
            if i < 2:
                board_str += "\n----------\n"
        return board_str
...
print("** Tic Tac Toe ** ")
print(f"The valid moves are: {Board.valid_moves}")

while not game_complete:
    print(str(board))
    game_complete, message = board.whats_next()
    ...
    print(message)
    next_move = input("Enter move: ").lower().strip()
